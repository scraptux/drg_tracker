<template>
  <div class="bg-white">
    <div v-if="loading==0 && !error" class="pt-6">
      <NavbarBreadcrumb class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8" :drg="$route.params.drg" :year="year" :kapnr="kapitel.KapNr"></NavbarBreadcrumb>
      ...
      <div class="mx-auto max-w-7xl space-x-2 py-5 sm:px-6 lg:px-3 border-y mb-4 border-gray-200">
        <table class="w-full table-auto">
          <tr @click="router.push(`/${$route.params.drg}/version/${year}/gruppe/${gruppe.GrVon}`)"
            ...
            <td class="py-4 px-6 text-sm text-center my-auto font-medium text-gray-500 whitespace-nowrap">{{ gruppe.GrVon }} - {{ gruppe.GrBis }}</td>
